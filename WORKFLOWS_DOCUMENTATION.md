...
Работното време се подава само през едно поле, например `WorkTime` в JSON или колоната `Работно време` в Excel. При подадени два реда програмата пази и двата прозореца. OR-Tools забранява паузата между прозорците, PyVRP моделира прозорците като алтернативни варианти на същия клиент, а TSP режимът изчаква до следващия възможен прозорец. Ако клиентът няма работно време, се използва default прозорецът от настройките. Ако `enable_customer_time_windows = False`, solver-ите игнорират тези прозорци, но работното време пак се показва в индивидуалните route карти.
