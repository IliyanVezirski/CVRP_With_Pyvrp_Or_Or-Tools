...
def build_ordered_depots(
    ...
    for vehicle_config in vehicle_configs or []:
        if not getattr(vehicle_config, "enabled", False):
            continue
        start_location = _normalise_coords(getattr(vehicle_config, "start_location", None))
        if start_location and _coord_key(start_location) not in seen:
            extra_depots.append(start_location)
            seen.add(_coord_key(start_location))
...
class CVRPConfig:
    ...
    pyvrp_ils_no_improvement: int = 300000
    ...
    def _create_default_vehicles(self) -> List[VehicleConfig]:
        ...
        return [
            VehicleConfig(
                ...
                start_location=(42.69735652560932, 23.323809998750914),
                start_time_minutes=480,
                tsp_depot_location=(42.69735652560932, 23.323809998750914)
            ),
            VehicleConfig(
